...
$$\eta_D={cos}^2\theta=\left[\frac{\int{\cos\funcapply(\theta)d\dot{m}}}{\int{d\dot{m}}}\right]^2$$
...
$$\eta_n=\frac{h_c-h_e-\sfrac{\dot{Q}}{\dot{m}}}{h_c-h_e}$$
...
$$\alpha_1=2\astfraction\ of\ molecules\ that\ have\ dissociated$$
...
$$\vec{v}=-\frac{E_o}{B_o}\hat{y}+v_\bot[\cos{\left(\omega_{ce}t+\theta\right)}\hat{x}+\sin\funcapply(\omega_{ce}t+\theta)\hat{y}]$$
...
$$\frac{q}{m}= \frac{f(\varepsilon\right)}{\rho} (\frac{\gamma K}{\varepsilon Q})^\frac{1}{2}$$
...
$$T=\frac{f(\varepsilon )^2}{\rho (g_oIsp\right)^3}(\frac{\gamma K}{\varepsilon}\right)4{V_o}^2$$
